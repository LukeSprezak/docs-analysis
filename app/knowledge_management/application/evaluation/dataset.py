...
def load_examples(path: str | Path) -> list[EvaluationExample]:
    """Loads the golden set from a JSON file (a list of objects) into the `EvaluationExample` list."""
    content = Path(path).read_text(encoding="utf-8")
    parsed = json.loads(content)
    if not isinstance(parsed, list):
        raise ValueError("The Golden set must be a list of JSON objects.")
